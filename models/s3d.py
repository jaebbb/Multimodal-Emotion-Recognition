# This code is from https://github.com/kylemin/S3D/blob/master/model.py
import torch
...
class S3D(nn.Module):
    def __init__(self, end_point=None, num_class=None):
        ...
        if end_point == 'mixed_5a':
            return
        self.base.add_module('14', Mixed_5b())

        if end_point == 'mixed_5b':
            return
        self.base.add_module('15', Mixed_5c())

        if end_point == 'mixed_5c' or end_point == 'avg_pool3d':
            return
    ...
    def forward(self, x):
        y = self.base(x)
        # Endpoint is before average pooling
        if self.end_point is not None and self.end_point != 'avg_pool3d':
            return y

        y = F.avg_pool3d(y, (2, y.size(3), y.size(4)), stride=1)
        # Endpoint is average pooling
        if self.end_point == 'avg_pool3d':
            return y
...
class SepConv3d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding=0):
        ...
        self.bn_s = nn.BatchNorm3d(
            out_planes, eps=1e-3, momentum=0.001, affine=True)
        self.relu_s = nn.ReLU()

        self.conv_t = nn.Conv3d(out_planes, out_planes, kernel_size=(
            kernel_size, 1, 1), stride=(stride, 1, 1), padding=(padding, 0, 0), bias=False)
        self.bn_t = nn.BatchNorm3d(
            out_planes, eps=1e-3, momentum=0.001, affine=True)
        self.relu_t = nn.ReLU()
